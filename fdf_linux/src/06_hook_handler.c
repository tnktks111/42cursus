...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/28 17:27:21 by ttanaka           #+#    #+#             */
/*   Updated: 2025/05/28 19:58:09 by ttanaka          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	hook_handler(t_env *env)
{
	mlx_hook(env->win_ptr, 2, (1L << 0), key_button_press, env);
	mlx_hook(env->win_ptr, 4, (1L << 2), handle_button_press, env);
	mlx_hook(env->win_ptr, 5, (1L << 3), handle_button_release, env);
	mlx_hook(env->win_ptr, 6, (1L << 6), handle_mouse_motion, env);
	mlx_hook(env->win_ptr, 17, (1L << 17), close_window, env);
}
...
int	handle_mouse_motion(int x, int y, void *param_env)
{
	...
	{
		env->v_curr = map_mouse_to_sphere(x, y, env);
