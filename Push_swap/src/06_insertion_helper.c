...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/21 12:41:14 by ttanaka           #+#    #+#             */
/*   Updated: 2025/05/21 13:04:24 by ttanaka          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

void	gen_score_info(t_list_info *info_a, t_list_info *info_b,
		t_score_info score_info[])
{
	...
	while (++i < info_b->size)
	{
		score_info[i].a_score = find_insert_point(l_a, info_a->size,
				curr->content);
		score_info[i].a_rev_score = -((info_a->size - score_info[i].a_score)
				% info_a->size);
...
void	calc_min_score(t_score_info *score_info)
{
	...
	res = ft_min(res, -ft_min(score_info->a_rev_score,
				score_info->b_rev_score));
...
void	decide_rotate_cnt(t_score_info *s_info)
{
	if ((s_info->min_score) == ft_max(s_info->a_score, s_info->b_score))
	{
		if (s_info->a_score > s_info->b_score)
			set_rotate_cnt(s_info, s_info->a_score - s_info->b_score, 0,
				s_info->b_score);
		else
			set_rotate_cnt(s_info, 0, s_info->b_score - s_info->a_score,
				s_info->a_score);
	}
	else if (s_info->min_score == -ft_min(s_info->a_rev_score,
			s_info->b_rev_score))
	{
		if (s_info->a_rev_score < s_info->b_rev_score)
			set_rotate_cnt(s_info, s_info->a_rev_score - s_info->b_rev_score, 0,
				s_info->b_rev_score);
		else
			set_rotate_cnt(s_info, 0, s_info->b_rev_score - s_info->a_rev_score,
				s_info->a_rev_score);
	}
