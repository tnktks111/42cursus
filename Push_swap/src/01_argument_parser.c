...
/*   Updated: 2025/05/21 14:12:46 by ttanaka          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_bool	already_sorted(t_list_info *info)
{
	long	*l;
	int		i;
...
int	argv_parser(int argc, char *argv[], t_list_info *info)
{
	...
	if (!info->head)
		return (EXIT_FAILURE);
	info->size = argc - 1;
	if (already_sorted(info) == True)
		return (free_all_node(info->head), EXIT_SORTED);
